  ...
  useEffect(() => {
    const fetchCabana = async () => {
      try {
        ...
        console.log('Respuesta detalle cabaña:', response.data);

        const cabanaData = response.data.data || response.data;

        // Validar si el objeto tiene los campos esperados
        if (!cabanaData || !cabanaData.id) {
